...
async def final_admin_panel_callback(update,context):
    ...
    if a=="health":
        ...
        await q.message.edit_text(health_text(),parse_mode="HTML",reply_markup=final_admin_keyboard())
        return
    ...
    if a=="health_run":
        await run_health_checks(context.bot,uid)
        await q.message.edit_text(health_text()+"\n\n🩺 چکاپ دستی انجام شد.", parse_mode="HTML", reply_markup=final_admin_keyboard())
        return
    if a=="test":
...
def _n8n_ai_fallback_sync(prompt):
    """Optional AI fallback. Sends only the prompt; never sends payments/ownership data."""
    if not AI_FAILOVER_TO_N8N or not N8N_WEBHOOK_URL or not N8N_API_KEY:
        return None
    if not N8N_WEBHOOK_URL.lower().startswith("https://"):
        return None
    payload=json.dumps({
        "event":"ai_fallback",
        "prompt":str(prompt)[:8000],
    },ensure_ascii=False).encode("utf-8")
    headers={"Content-Type":"application/json","X-MyTasks-Key":N8N_API_KEY}
    try:
        ...
        with urllib.request.urlopen(req,timeout=N8N_TIMEOUT) as resp:
            data=json.loads(resp.read().decode("utf-8"))
        answer=(data.get("output_text") or data.get("answer") or data.get("text") or "").strip()
        if answer:
            _record_service_event("n8n","OK","AI fallback")
            return answer[:4000]
    except Exception as exc:
        _record_service_event("n8n","ERROR",type(exc).__name__)
        logger.warning("n8n AI fallback failed: %s", type(exc).__name__)
    return None
...
def n8n_configured():
    return bool(_secure_remote_base(N8N_WEBHOOK_URL) and N8N_API_KEY)
...
async def ai_chat_text(update,context):
    ...
    try:
        answer=None
        # Preferred path: self-hosted OmniRoute gateway.
        ...
        if not answer and api_key:
            ...
            if answer:
                _record_service_event("openai","OK","chat")
        if not answer:
            _record_service_event("openai","WARN","chat fallback")
...
async def run_health_checks(bot,admin_id=0):
    ...
    checks.append(("n8n","OK" if n8n_ok else "WARN",
                   "درگاه n8n برای Automation/AI fallback تنظیم شده است."
                   if n8n_ok else
                   "N8N_WEBHOOK_URL/N8N_API_KEY تنظیم نشده است؛ ربات بدون n8n ادامه می‌دهد."))
    ...
    elif ai_key or n8n_ok or omniroute_configured():
        providers=[]
        if omniroute_configured(): providers.append("OmniRoute")
        if ai_key: providers.append("OpenAI")
        if n8n_ok: providers.append("n8n fallback")
        checks.append(("AI","OK","AI فعال است؛ مسیرها: " + " + ".join(providers)))
    else:
        checks.append(("AI","WARN","AI فعال است ولی هیچ ارائه‌دهنده AI تنظیم نشده است."))
