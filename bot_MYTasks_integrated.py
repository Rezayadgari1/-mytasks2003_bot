...
def n8n_configured():
    return bool(N8N_WEBHOOK_URL and N8N_API_KEY and N8N_WEBHOOK_URL.lower().startswith("https://"))
...
async def ai_chat_start(update,context):
    ...
    if not allowed:
        ...
        return
    api_key=os.environ.get("OPENAI_API_KEY","").strip()
    if not api_key and not n8n_configured():
        clear_flow(context)
        await update.message.reply_text(
...
async def run_health_checks(bot,admin_id=0):
    ...
    elif ai_key or n8n_ok:
        providers=[]
        if ai_key: providers.append("OpenAI")
        if n8n_ok: providers.append("n8n fallback")
        checks.append(("AI","OK","AI فعال است؛ ارائه‌دهنده: " + " + ".join(providers)))
    else:
